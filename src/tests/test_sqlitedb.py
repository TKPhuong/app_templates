...
def test_db():
    db = SqliteDB(DB_FILE)
    db.add_table("users", [("id", "INTEGER PRIMARY KEY"), ("name", "TEXT"), ("age", "INTEGER")])
...
class TestSqliteDB:
    ...
    def test_select(self, test_db):
        result = test_db.select("users", columns=["name", "age"], where_clause="age > 20")
    ...
    def test_join(self, test_db):
        db = test_db
        db.add_table("books", [("id", "INTEGER PRIMARY KEY"), ("title", "TEXT"), ("user_id", "INTEGER")])
        db.insert("books", [1, "The Great Gatsby", 1])
        db.insert("books", [2, "Pride and Prejudice", 1])
        db.insert("books", [3, "To Kill a Mockingbird", 2])
        result = db.select("users", columns=["name", "title"], join_clause="JOIN books ON users.id = books.user_id")
        assert result == [("Alice", "The Great Gatsby"), ("Alice", "Pride and Prejudice"), ("Bob", "To Kill a Mockingbird")]